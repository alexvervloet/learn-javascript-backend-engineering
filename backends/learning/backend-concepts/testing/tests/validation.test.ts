...
describe("POST /posts validation", () => {
  test("missing title → 422", async () => {
    const user = makeUser();
    expect((await api().post("/posts").set(auth(user.id)).send({ body: "content" })).status).toBe(422);
  });

  test("missing body → 422", async () => {
    const user = makeUser();
    expect((await api().post("/posts").set(auth(user.id)).send({ title: "A title" })).status).toBe(422);
  });

  test("empty title → 422", async () => {
    const user = makeUser();
    expect((await api().post("/posts").set(auth(user.id)).send({ title: "", body: "content" })).status).toBe(422);
  });

  test("empty body → 422", async () => {
    const user = makeUser();
    expect((await api().post("/posts").set(auth(user.id)).send({ title: "A title", body: "" })).status).toBe(422);
  });

  test("title at max length (200) is accepted", async () => {
    const user = makeUser();
    expect((await api().post("/posts").set(auth(user.id)).send({ title: "x".repeat(200), body: "c" })).status).toBe(201);
  });

  test("title exceeding max length (201) → 422", async () => {
    const user = makeUser();
    expect((await api().post("/posts").set(auth(user.id)).send({ title: "x".repeat(201), body: "c" })).status).toBe(422);
  });

  test("no body at all → 422", async () => {
    const user = makeUser();
    expect((await api().post("/posts").set(auth(user.id))).status).toBe(422);
  });
  ...
  test.each([
    ...
    const user = makeUser();
    const res = await api().post("/posts").set(auth(user.id)).send({ title, body: "content" });
...
describe("PATCH /posts/:id validation", () => {
  test("empty title → 422", async () => {
    ...
    expect((await api().patch(`/posts/${post.id}`).set(auth(user.id)).send({ title: "" })).status).toBe(422);
  });

  test("title exceeding max length → 422", async () => {
    const user = makeUser();
    const post = makePost(user);
    expect((await api().patch(`/posts/${post.id}`).set(auth(user.id)).send({ title: "x".repeat(201) })).status).toBe(422);
  });

  test("empty object {} is valid (all fields optional)", async () => {
    const user = makeUser();
    const post = makePost(user);
    expect((await api().patch(`/posts/${post.id}`).set(auth(user.id)).send({})).status).toBe(200);
  });

  test("explicit null for an optional field is accepted", async () => {
    const user = makeUser();
    const post = makePost(user);
    expect((await api().patch(`/posts/${post.id}`).set(auth(user.id)).send({ title: null })).status).toBe(200);
  });

  test("wrong type for published → 422 (Zod is strict)", async () => {
    const user = makeUser();
    const post = makePost(user);
    expect((await api().patch(`/posts/${post.id}`).set(auth(user.id)).send({ published: "yes" })).status).toBe(422);
  });
});
