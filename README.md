...
- [backend-concepts](backends/learning/backend-concepts/) — Node runtime internals (event loop, streams, backpressure, workers, cluster), web security (headers, injection, XSS, CSRF), auth, caching, rate limiting, pagination, webhooks, WebSockets, Kafka, OAuth2, observability (logs, metrics, OpenTelemetry traces)
